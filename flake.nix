  ...
  outputs = { self, nixpkgs, flake-utils, clj-nix }:
    flake-utils.lib.eachDefaultSystem
      (system:
        ...
        in
        {
          nixosModules.marrano-bot = { config, pkgs, lib, ... }:
            with lib;
            let
          ...
          packages = {
            marrano-bot = cljpkgs.mkCljBin {
              projectSrc = ./.;
              name = "marrano-bot";
              main-ns = "moolite.bot.server";
              java-opts = [ "-Djava.awt.headless=true" ];
              buildInputs = [ pkgs.clojure pkgs.git ];
              buildCommand = "clojure -X:build uber";
            };

            marrano-bot-graal = cljpkgs.mkGraalBin {
              cljDrv = self.packages."${system}".marrano-bot;
            };
